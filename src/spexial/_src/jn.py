...
def check_float64_enabled():
    if not config.read("jax_enable_x64"):
        warnings.warn("float64 is not enabled in JAX. This may lead to reduced precision.")
...
def _besseljn_backward_recurrence(
    z,
    v: int,
    n_iter: int
):
    ...
    (f0, f1, bs, _, _f) = lax.fori_loop(
        jnp.zeros_like(n_iter), n_iter + 1, body_fun, (f0, f1, bs, z, _f)
    )
...
@Jn.defjvp
def Jn_jvp(v, primals, tangents):
    """ """
    (z,), (zdot,) = primals, tangents
    primal_out = Jn(v, z)
    tangent_out = (Jn(v - 1, z) - Jn(v + 1, z)) / 2
